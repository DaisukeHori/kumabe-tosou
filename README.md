...
contact.html    … 08 相談する（見積り3変数・受付窓口・FAQ）
...
イメージ写真は Wikimedia Commons の商用利用可能な素材（CC BY / CC0 / パブリックドメイン）を使用しています。ライセンスと作者は `assets/img/CREDITS.json` に記録し、各写真のキャプションにもクレジットを明記しています。CC BY 素材は作者クレジットを表示することで利用条件を満たしています。

**注意**: これらは仮のイメージ写真です。本番では、隈部塗装の実際の工房・設備・制作事例の写真への差し替えを推奨します。代表ポートレートは、実在性の観点から本人写真の掲載までプレースホルダとしています。

配置箇所: ホーム（ヒーローバンド＋3枚）、ストーリー（1枚）、会社案内（設備3枚）、サービス（工程2枚）、工程（3枚）、素材対応（3Dプリント2枚）、色見本（1枚）。
