...
// vi.mock はファイル先頭へホイストされるため、以下のファクトリから参照する可変値は
// vi.hoisted() で包む (tests/visual-suggest-text.test.ts 等の precedent と同型)。
const { OPS_LIMITS, opsLimitsRow } = vi.hoisted(() => {
  const OPS_LIMITS = {
  ...
  return {
    OPS_LIMITS,
    // テストごとに site_settings.ops_limits 読み取りの応答を差し替えるための可変箱
    // (missing/invalid/exceeded ブランチの単体化 — 敵対レビュー MAJOR#1)。
    opsLimitsRow: {
      current: { data: { value: OPS_LIMITS } as { value: unknown } | null, error: null as { message: string } | null },
    },
  };
});
...
// getOpsLimitsForService) は migration 20260711000021 適用後、settingsFacade
// (cookie セッション前提) を経由せず、注入された service client で site_settings を直接読む
// (#41 敵対レビュー BLOCKER 修正)。テストダブルも site_settings.ops_limits の読み取りチェーンを
// 模倣する (他テーブルはすべて repository モジュール側の mock 経由で完結し、この client の
// メソッドを実際には呼ばないため from("site_settings") のみ対応すれば十分)。
// opsLimitsRow.current は beforeEach で既定値にリセットしつつ、missing/invalid/exceeded を
// 検証するテストケースが個別に上書きする。
...
beforeEach(() => {
  ...
  opsLimitsRow.current = { data: { value: OPS_LIMITS }, error: null };
...
describe("checkXBillingGuardExceeded (runPublishWorkerBatch 経由): fail-closed / 読取不能ブランチ", () => {
  it("ops_limits 行が存在しない (missing) → X API を一切呼ばず KMB-E901 で markFailed される", async () => {
    ...
    fetchMock.mockImplementation(async (input: string | URL) => {
      throw new Error(`ops_limits 読取不能時は X API を呼んではいけない: ${String(input)}`);
    });
    ...
    expect(markFailed).toHaveBeenCalledTimes(1);
    const call = markFailed.mock.calls[0];
    // 真の上限超過 (KMB-E505) と誤読されないよう、読取不能は KMB-E901 として区別される
    expect(call[2]).toMatchObject({ code: "KMB-E901" });
    expect(String(call[2].detail)).toContain("ops_limits");
    expect(markManualRequired).not.toHaveBeenCalled();
    expect(markPublished).not.toHaveBeenCalled();
  });

  it("ops_limits の value が zOpsLimits と一致しない (invalid) → KMB-E901 で markFailed される (KMB-E505 にはしない)", async () => {
    ...
    const call = markFailed.mock.calls[0];
    expect(call[2]).toMatchObject({ code: "KMB-E901" });
    expect(call[2].code).not.toBe("KMB-E505");
    expect(markManualRequired).not.toHaveBeenCalled();
    expect(markPublished).not.toHaveBeenCalled();
  });

  it("ops_limits は正常に読めるが当月合算が上限を超過している → KMB-E505 で markFailed される (X API は呼ばない)", async () => {
    opsLimitsRow.current = {
      data: { value: { ...OPS_LIMITS, x_monthly_post_limit: 10 } },
      error: null,
    };
    getMonthlyXCostCentsSum.mockResolvedValue({ ok: true, value: 11 });
    const post = basePost();
    claimDueScheduledPosts.mockResolvedValue({ ok: true, value: [post] });
    getApprovedDraft.mockResolvedValue({ ok: true, value: draftWithThread([{ text: "hello", media_id: null }]) });

    fetchMock.mockImplementation(async (input: string | URL) => {
      throw new Error(`上限超過時は X API を呼んではいけない: ${String(input)}`);
    });
    ...
    expect(markFailed).toHaveBeenCalledTimes(1);
    const call = markFailed.mock.calls[0];
    expect(call[2]).toMatchObject({ code: "KMB-E505" });
    expect(markManualRequired).not.toHaveBeenCalled();
    expect(markPublished).not.toHaveBeenCalled();
  });

  it("ops_limits が正常かつ上限内 (境界値ちょうど) → ブロックされず通常どおり published される", async () => {
    opsLimitsRow.current = {
      data: { value: { ...OPS_LIMITS, x_monthly_post_limit: 10 } },
      error: null,
    };
    getMonthlyXCostCentsSum.mockResolvedValue({ ok: true, value: 10 }); // ちょうど上限 → ブロックしない
    const post = basePost();
    claimDueScheduledPosts.mockResolvedValue({ ok: true, value: [post] });
    getApprovedDraft.mockResolvedValue({ ok: true, value: draftWithThread([{ text: "hello", media_id: null }]) });
