...
class FakeChain implements PromiseLike<PgResult> {
  ...
  // #51 追加: listExpiringQuotes (lte) / listPaymentsForDocuments (in) が使う。
  lte(...a: unknown[]): this {
    return this.record("lte", a);
  }
  in(...a: unknown[]): this {
...
// ============================================================
// listExpiringQuotes / listUnpaidInvoices / listPaymentsForDocuments / bulkExpireOverdueQuotes
// (#51 — getSalesDigest / markExpiredQuotes の DB アクセス層)
// ============================================================

describe("listExpiringQuotes (documents: doc_type='quote' AND status='issued' AND valid_until<=cutoff)", () => {
  it("成功時は行を ExpiringQuoteRow へマッピングして返す", async () => {
    const row = { id: "d-1", doc_no: "Q-2026-0001", billing_name: "サンプル建設", valid_until: "2026-07-20", total_jpy: 22000 };
    const chain = new FakeChain({ data: [row], error: null });
    const { client } = buildClient({ fromQueue: [chain] });

    const result = await listExpiringQuotes(client, "2026-07-21");

    expect(result).toEqual({
      ok: true,
      value: [{ document_id: "d-1", doc_no: "Q-2026-0001", billing_name: "サンプル建設", valid_until: "2026-07-20", total_jpy: 22000 }],
    });
    const lteCall = chain.calls.find((c) => c.method === "lte");
    expect(lteCall?.args).toEqual(["valid_until", "2026-07-21"]);
  });

  it("data が null の場合は空配列を返す (握り潰しではなく PostgREST の 0 件表現)", async () => {
    const { client } = buildClient({ fromQueue: [new FakeChain({ data: null, error: null })] });
    const result = await listExpiringQuotes(client, "2026-07-21");
    expect(result).toEqual({ ok: true, value: [] });
  });

  it("エラーは握り潰さず Result.code へ変換して返す", async () => {
...
describe("listUnpaidInvoices (documents: doc_type='invoice' AND status='issued')", () => {
  it("成功時は行を UnpaidInvoiceRow へマッピングして返す (paid_jpy/balance_jpy はここでは算出しない — facade 側の責務)", async () => {
    const row = { id: "d-2", doc_no: "I-2026-0001", billing_name: "サンプル建設", issue_date: "2026-07-01", total_jpy: 11000 };
    const { client, fromCalls } = buildClient({ fromQueue: [new FakeChain({ data: [row], error: null })] });

    const result = await listUnpaidInvoices(client);

    expect(fromCalls).toEqual(["documents"]);
    expect(result).toEqual({
      ok: true,
      value: [{ document_id: "d-2", doc_no: "I-2026-0001", billing_name: "サンプル建設", issue_date: "2026-07-01", total_jpy: 11000 }],
    });
  });
...
describe("listPaymentsForDocuments (payments: document_id IN (...) — N+1 回避の一括取得)", () => {
  it("document_id 群を in 句へ渡し、成功時は PaymentRow[] をそのまま返す", async () => {
    const row = { id: "p-1", document_id: "d-2", paid_on: "2026-07-05", amount_jpy: 4000, method: "bank_transfer", memo: null, created_by: null, created_at: "t" };
    const chain = new FakeChain({ data: [row], error: null });
    const { client } = buildClient({ fromQueue: [chain] });

    const result = await listPaymentsForDocuments(client, ["d-2", "d-3"]);

    expect(result).toEqual({ ok: true, value: [row] });
    const inCall = chain.calls.find((c) => c.method === "in");
    expect(inCall?.args).toEqual(["document_id", ["d-2", "d-3"]]);
  });

  it("エラーは握り潰さず Result.code へ変換して返す", async () => {
    const { client } = buildClient({ fromQueue: [new FakeChain({ data: null, error: { message: "denied" } })] });
    const result = await listPaymentsForDocuments(client, ["d-2"]);
    expect(result).toEqual({ ok: false, code: "KMB-E901", detail: "denied" });
  });
});

describe("bulkExpireOverdueQuotes (documents: 一括UPDATE、CAS なし — updateDocumentStatusWithCas とは別関数)", () => {
  it("成功時は valid_until < todayDateOnly の issued 見積を status='expired' に更新し、更新された全行を返す", async () => {
    const updatedRow = { id: "d-1", doc_type: "quote", status: "expired", status_reason: "有効期限切れ(自動判定)" };
    const chain = new FakeChain({ data: [updatedRow], error: null });
    const { client } = buildClient({ fromQueue: [chain] });

    const result = await bulkExpireOverdueQuotes(client, "2026-07-14");

    expect(result).toEqual({ ok: true, value: [updatedRow] });
    const updateCall = chain.calls.find((c) => c.method === "update");
    expect(updateCall?.args).toEqual([{ status: "expired", status_reason: "有効期限切れ(自動判定)" }]);
    const ltCall = chain.calls.find((c) => c.method === "lt");
    expect(ltCall?.args).toEqual(["valid_until", "2026-07-14"]);
  });

  it("対象0件 (data:[]) は空配列を返す (握り潰さない — 呼び出し元がループを回さないだけで正常系)", async () => {
    const { client } = buildClient({ fromQueue: [new FakeChain({ data: [], error: null })] });
    const result = await bulkExpireOverdueQuotes(client, "2026-07-14");
    expect(result).toEqual({ ok: true, value: [] });
  });
