...
import { msCalendarAdapter } from "@/modules/scheduling/internal/ms-api";
...
// #55 (Microsoft) 追加分。既存 (google) の定数/フィクスチャは無変更のまま、provider=microsoft
...
// ===========================================================================
// #55 (Microsoft) 追加分。既存 (google) の describe/it は無改変。
// canonical §8.5 の Graph 固有拡張 (時間窓必須 / 安全弁 E725) を sync-engine.ts の
// runPullLoop レベルで検証する (ms-api.ts 自体の pullChanges 中継は
// tests/scheduling-ms-api.test.ts が担当)。
// ===========================================================================

function msConnectionRow(overrides: Partial<CalendarConnectionRow> = {}): CalendarConnectionRow {
  return connectionRow({
    provider: "microsoft",
    vault_secret_name: "calendar_microsoft_oauth",
    meta: {
      account_email: "owner@example.com",
      app_calendar_id: CAL_ID,
      token_expires_at: null,
      sync_window_start: "2026-06-11",
      sync_window_end: "2026-12-08",
    },
    ...overrides,
  });
}

describe("runPull (provider=microsoft, #55): 初回フル同期は meta.sync_window_start/end を calendarView/delta の時間窓として渡す", () => {
  it("token/cursor が未設定の初回同期は startDateTime/endDateTime を meta の窓から補って呼び出す (§1.4/§8.1 時間窓必須)", async () => {
    ...
    recordAndRoute((url, method) => {
      if (url.startsWith(`${GRAPH_CAL_BASE}/calendarView/delta?`) && method === "GET") {
        ...
        expect(parsed.searchParams.get("startDateTime")).toBe("2026-06-11T00:00:00Z");
        expect(parsed.searchParams.get("endDateTime")).toBe("2026-12-08T00:00:00Z");
        return jsonResponse(200, { value: [], "@odata.deltaLink": `${GRAPH_CAL_BASE}/calendarView/delta?$deltatoken=fresh` });
      }
      throw new Error(`unexpected call: ${method} ${url}`);
    });

    const result = await runPull(FAKE_CLIENT, "microsoft", msCalendarAdapter);
    ...
    expect(repoMocks.updateCalendarConnectionAfterPull).toHaveBeenCalledWith(
      ...
      "microsoft",
      expect.objectContaining({
        sync_token: `${GRAPH_CAL_BASE}/calendarView/delta?$deltatoken=fresh`,
        sync_page_cursor: null,
      }),
    );
  });
...
describe("runPull (provider=microsoft, #55): deltaLink 410 (Gone) → KMB-E722 でフル再同期 (再試行にも時間窓を付与)", () => {
  it("410 検知後、token/cursor を破棄してフル再同期を即時再試行し、再試行のリクエストにも時間窓を付ける", async () => {
    const staleDeltaLink = `${GRAPH_CAL_BASE}/calendarView/delta?$deltatoken=stale-410`;
    repoMocks.getCalendarConnection.mockResolvedValue({
      ok: true,
      value: msConnectionRow({ sync_token: staleDeltaLink }),
    });
    let callIndex = 0;
    recordAndRoute((url, method) => {
      if (method !== "GET") throw new Error(`unexpected method: ${method} ${url}`);
      callIndex += 1;
      if (callIndex === 1) {
        expect(url).toBe(staleDeltaLink);
        return new Response("gone", { status: 410 });
      }
      // 2回目 (トークン/カーソル NULL 化後の再試行) — 初回同様、時間窓を付けて呼び直す
      expect(url.startsWith(`${GRAPH_CAL_BASE}/calendarView/delta?`)).toBe(true);
      expect(new URL(url).searchParams.get("startDateTime")).toBe("2026-06-11T00:00:00Z");
      return jsonResponse(200, { value: [], "@odata.deltaLink": `${GRAPH_CAL_BASE}/calendarView/delta?$deltatoken=brand-new-ms` });
    });
    ...
    expect(repoMocks.updateCalendarConnectionAfterPull).toHaveBeenCalledWith(
      ...
      "microsoft",
      expect.objectContaining({
        sync_token: `${GRAPH_CAL_BASE}/calendarView/delta?$deltatoken=brand-new-ms`,
        sync_page_cursor: null,
        last_error_code: "KMB-E722",
      }),
    );
...
describe("runPull (provider=microsoft, #55): 同一 skiptoken (nextLink) が2回連続 → KMB-E725 で中断 (P22 安全弁)", () => {
  it("2回目のページが1回目と同じ nextLink を返したら3回目を呼ばずに中断し、token/cursor を破棄する", async () => {
    const staleDeltaLink = `${GRAPH_CAL_BASE}/calendarView/delta?$deltatoken=stale-ms-2`;
    const repeatedNextLink = `${GRAPH_CAL_BASE}/calendarView/delta?$skiptoken=stuck`;
    repoMocks.getCalendarConnection.mockResolvedValue({
      ok: true,
      value: msConnectionRow({ sync_token: staleDeltaLink }),
    });
    recordAndRoute((url, method) => {
      if (url.startsWith(GRAPH_CAL_BASE) && method === "GET") {
        return jsonResponse(200, { value: [], "@odata.nextLink": repeatedNextLink });
      }
      throw new Error(`unexpected call (安全弁が3回目のページを呼んでしまった疑い): ${method} ${url}`);
    });

    const result = await runPull(FAKE_CLIENT, "microsoft", msCalendarAdapter);

    expect(calls).toHaveLength(2); // 同一 nextLink が2回目に検知された時点で3回目を呼ばない
    expect(result.full_resync).toBe(false); // 410 経由ではないフル再同期と区別する
    expect(repoMocks.updateCalendarConnectionAfterPull).toHaveBeenCalledWith(
      FAKE_CLIENT,
      "microsoft",
      expect.objectContaining({ sync_token: null, sync_page_cursor: null, last_error_code: "KMB-E725" }),
    );
  });
});

describe("runPull (provider=microsoft, #55): ページ上限に達してもラウンド未完了なら KMB-E725", () => {
  it("maxPages 到達まで異なる nextLink を返し続けても上限で打ち切り、E725 で token/cursor を破棄する", async () => {
    const staleDeltaLink = `${GRAPH_CAL_BASE}/calendarView/delta?$deltatoken=stale-ms-3`;
    repoMocks.getCalendarConnection.mockResolvedValue({
      ok: true,
      value: msConnectionRow({ sync_token: staleDeltaLink }),
    });
    ...
    recordAndRoute((url, method) => {
      if (url.startsWith(GRAPH_CAL_BASE) && method === "GET") {
        pageIndex += 1;
        return jsonResponse(200, { value: [], "@odata.nextLink": `${GRAPH_CAL_BASE}/calendarView/delta?$skiptoken=page-${pageIndex}` });
      }
      throw new Error(`unexpected call: ${method} ${url}`);
    });

    const result = await runPull(FAKE_CLIENT, "microsoft", msCalendarAdapter, { maxPages: 3 });

    expect(calls).toHaveLength(3); // maxPages で打ち切られ、無限にページングし続けない
    expect(result.full_resync).toBe(false);
    expect(repoMocks.updateCalendarConnectionAfterPull).toHaveBeenCalledWith(
      FAKE_CLIENT,
      "microsoft",
      expect.objectContaining({ sync_token: null, sync_page_cursor: null, last_error_code: "KMB-E725" }),
...
describe("runPull (provider=google, #55 回帰確認): ページ上限に達してもラウンド未完了のとき KMB-E725 は発火しない", () => {
  it("Graph 安全弁は provider==='microsoft' 限定であり、google はページ上限到達だけでは E725 化されない (既存 Google 挙動の固定)", async () => {
    let pageIndex = 0;
    recordAndRoute((url, method) => {
      if (url.startsWith(`${CAL_BASE}/events?`) && method === "GET") {
        pageIndex += 1;
        return jsonResponse(200, { items: [], nextPageToken: `page-${pageIndex}` });
      }
      throw new Error(`unexpected call: ${method} ${url}`);
    });

    const result = await runPull(FAKE_CLIENT, "google", googleCalendarAdapter, { maxPages: 3 });
