...
// internal/ai-studio-bridge.ts の resolveAiStudioFacade は「動的 import + 名前付きエクスポート
// 有無チェック」の薄いラッパー (ファイル冒頭のコメント参照)。ここを実体のまま (動的 import) にすると、
// 並列 createNoteDraft のシミュレーションテストで同一モジュールへの動的 import が同時に 2 回発火し、
// Vitest の SSR モジュールランナー側の既知の挙動でモック未適用のモジュールが返ることがあり
// (テストハーネス側の問題で本番の Vercel serverless 実行には無関係)、CAS の排他検証と無関係な
// 理由でフレーキーになる。静的 import であるこのモジュール自体を直接モックすることで回避する。
...
describe("createNoteDraft: 前提条件のバリデーション", () => {
  ...
  it("note セッション Cookie 未登録は KMB-E409 (未登録は明示失敗と別扱い、状態は変更しない)", async () => {
    ...
    expect(claimNoteDraftCreating).not.toHaveBeenCalled();
...
describe("createNoteDraft: 状態遷移 (§8 MAJOR-3)", () => {
  it("成功 → created + URL を保存・返却する", async () => {
    ...
    expect(claimNoteDraftCreating).toHaveBeenCalledWith(expect.anything(), "post-1");
...
describe("createNoteDraft: unknown からの再試行時の重複防止照合 (§8 MAJOR-3)", () => {
  ...
  it("note_draft_status='creating' で入ってきた場合は CAS (none/failed/unknown→creating) が必ず 0 行になるため、reconcile も新規作成 API も実行せず早期リターンする (§8 MAJOR-3 の二重作成バグを実装レビューで発見・修正。旧仕様は creating からも reconcile 経由で新規作成 API へ到達しており、並列呼び出しで二重下書きを作りえた)", async () => {
    // 'creating' は「別プロセスが今まさに作成中」か「前回プロセスがクラッシュし成否未確定」の
    // いずれかで区別できないため、CAS の遷移元集合 (none/failed/unknown) に含めない。
    // repository.claimNoteDraftCreating は実 DB では現在値が 'creating' の場合 0 行更新 (=false) を
    // 返す — ここではその挙動をモックで再現する。
...
describe("createNoteDraft: CAS 排他による二重作成防止 (実装レビューで発見・修正)", () => {
  it("CAS が 0 行 (=既に他プロセスが creating に遷移済み) の場合、外部 note API を呼ばず早期リターンする", async () => {
    getChannelPostById.mockResolvedValue({ ok: true, value: basePost({ note_draft_status: "none" }) });
    vaultReadSecret.mockResolvedValue({ ok: true, value: "_note_session_v5=abc" });
    claimNoteDraftCreating.mockResolvedValue({ ok: true, value: false });
  ...
  it("並列呼び出しシミュレーション: 同一 post に対する 2 回の createNoteDraft が同時に来ても、CAS で 1 つだけが creating を勝ち取り、外部 note API は 1 回しか呼ばれない (Codex 指摘の二重作成バグの修正確認)", async () => {
    getChannelPostById.mockResolvedValue({ ok: true, value: basePost({ note_draft_status: "none" }) });
    vaultReadSecret.mockResolvedValue({ ok: true, value: "_note_session_v5=abc" });

    // repository.claimNoteDraftCreating の DB 側 CAS (`update ... where note_draft_status in
    // ('none','failed','unknown') returning ...`) を、共有フラグを使った同期的な排他チェックで
    // 模擬する。JS の関数呼び出しは (async 関数内に await が無ければ) 常に同期的に実行完了するため、
    // 2 つの createNoteDraft 呼び出しがどうインターリーブしても「先に呼ばれた方だけが勝つ」という
    // DB の原子性を正しく再現できる。
    let claimed = false;
    claimNoteDraftCreating.mockImplementation(async () => {
      if (claimed) return { ok: true, value: false };
      claimed = true;
    ...
    callNoteCreateDraftApi.mockResolvedValue({
      kind: "created",
      draftId: "555",
      url: "https://note.com/notes/555/edit",
      headerImageWarning: null,
    });

    const [resultA, resultB] = await Promise.all([
      distributionFacade.createNoteDraft("post-1"),
      distributionFacade.createNoteDraft("post-1"),
    ]);

    // 外部 note API (実際の下書き作成) は CAS を勝ち取った 1 回のみ呼ばれる = 二重作成されない
    expect(callNoteCreateDraftApi).toHaveBeenCalledTimes(1);
    expect(claimNoteDraftCreating).toHaveBeenCalledTimes(2);

    const results = [resultA, resultB];
    const winners = results.filter((r) => r.ok && r.value.status === "created");
    const losers = results.filter((r) => r.ok && r.value.status === "creating");
    expect(winners).toHaveLength(1);
    expect(losers).toHaveLength(1);
    expect(winners[0]).toEqual({ ok: true, value: { status: "created", url: "https://note.com/notes/555/edit" } });
    expect(losers[0]).toEqual({ ok: true, value: { status: "creating", url: null } });
  });
});
