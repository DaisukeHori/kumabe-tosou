...
- **v1 対象 = Tier A 75 スロット** (PLAN.md §3.2)。DB 由来テキスト (works/posts/voices、site_settings) は対象外 — 既存フォームが正
...
export const TEXT_REGISTRY: readonly PageTextSlot[] = [ /* 75 件 (PLAN.md §3.2 と 1:1) */ ];
...
setText(slotKey: string, text: string | null): Promise<Result<void>>; // null = 既定に戻す (行削除)
...
- Server Action `setSlotText(slotKey, text | null)`: Zod parse (registry 限定 + maxLen + kind 検証) → requireAdmin → facade.setText → **commit 後 return 前に** `revalidatePath(route)` + `revalidateTag("page_text")` (shared.* スロットは全静的 route を revalidate)
...
| レイヤ | 対象 |
|---|---|
| 単体 | TEXT_REGISTRY: 75 件・key 一意・route 妥当・**defaultText が page-body の現行文言と一致** (転記ミス検出) / resolver fallback + JSON round-trip / Zod (maxLen 境界・kind=text の改行拒否・registry 外) / textEditableAttrs の editMode 出し分け |
