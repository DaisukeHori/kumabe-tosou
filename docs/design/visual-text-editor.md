...
- 検証: registry 外 slot_key → KMB-E107 / maxLen 超過・kind 違反 (text に改行等)・**空文字列 (min 1、全スロットで空は無意味)** → KMB-E101。保存前に **`\r\n?` → `\n` 正規化** (貼り付け由来の CR 混入を保存側で吸収 — UI 正規化に依存しない)
...
| v1.3 | 2026-07-10 | T1 tester 指摘反映: 空文字列拒否 (min 1) + 保存側 CRLF 正規化を §3 検証規則に追加 |
