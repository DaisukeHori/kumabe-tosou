# 山岸塗装 CRM スイート — crm モジュール設計書 (01-crm)

- 版: v1.4 (2026-07-16: **顧客の請求先/配送先 (Issue #113)** — customers.billing_info/shipping_info jsonb 2 列 ({postal_code, address, tel_e164, name, suffix}) を契約外拡張で追加。郵便番号→住所の自動補完 (zipcloud プロキシ)・帳票初期値連携。migration 20260715000002・§2.2 追記・§2.6 対応表・§5.2 zPostalCode7/zCustomerAddressBlock・§5.3 CustomerDetail・§6.4 マージのブロック coalesce・§7.1 lookupPostalAddressAction・§8.2 編集 Sheet 2 セクション+コピー+補完+suffix・§9 KMB-E610/E611。跨モジュール射影 CustomerRef/DealRef の billing/shipping は 07-contracts-delta v1.10、帳票複製規則は 02-sales §6.1。canonical 契約 (zCustomerInput / CrmFacade シグネチャ) は無変更。変更点は末尾更新履歴)。旧: v1.3 (2026-07-15: **顧客カスタム項目 (Issue #98)** — customers.custom_fields jsonb (順序保持の {label,value} ペア配列) を契約外拡張のみで追加。migration 20260715000001・§2.2 追記・§5.2 zCustomerCustomFields・§5.3 CustomerDetail・§8.2 編集 Sheet「追加情報」セクション。canonical 契約 (zCustomerInput / CrmFacade シグネチャ) は無変更。変更点は末尾更新履歴)。旧: v1.2 (2026-07-11: **07-contracts-delta v1.2〜v1.7 追随** — relinkActivity の実装意味論 (§6.7 新設)・getCustomerRef/getDealRef/getDealRefs の契約メソッド表追記・§1.2 の ai-providers 将来枠記述の stale 是正。変更点は末尾更新履歴)。旧: v1.1 (2026-07-11: レビュー反映版 — 00-overview v1.0 / 07-contracts-delta v1.1 準拠)
...
| 項目 | 理由・扱い |
|---|---|
| メール連携 (activity_type 'email' の inbound — 受信取込/BCCログ) | 裁定 J7。outbound (帳票のメール送付) は issue #101 で段階解禁済み — `appendActivity` は payload.direction で判定し、inbound のみ引き続き挿入拒否 (KMB-E604)。受信基盤が無いため inbound は孤児データになる |
...
create table activities (
  ...
  )),  -- 'email' は J7 Phase 2 (check には元から含めていたため migration 不要のまま解禁可能だった)。
       -- issue #101 で outbound (帳票のメール送付) を解禁。inbound (受信取込) は引き続き facade が拒否 (KMB-E604)
...
**v1.4 追記 (migration `20260715000002_customers_billing_shipping.sql` — Issue #113 顧客の請求先/配送先)**:
0023 の customers 定義・0001/0002 の履歴は不変。前提: 20260715000001 (custom_fields) 適用済み。以下は追加差分 (冪等 — 新規 migration ファイルとして適用)。**canonical DDL 全文は Issue #113 §1.2 に提示済み**。本節はその要旨:

```sql
alter table customers
  add column if not exists billing_info jsonb,   -- 請求先ブロック (NULL = 未設定)
  add column if not exists shipping_info jsonb;  -- 配送先 (施工先) ブロック

-- comment on column: 構造 {postal_code(7桁数字), address, tel_e164, name, suffix(様/御中)}。
-- 形式・上限は Zod (§5.2 zCustomerAddressBlock) のみが正、check は構造整合のみ。
-- billing_info の複製規則は 02-sales.md §6.1 (帳票 billing_* / site_* の初期値の源)
...
`crm_merge_customers` は署名不変 (uuid, uuid, timestamptz) のまま全文差し替え。custom_fields ブロックはそのまま、「勝者の空欄のみ敗者から補完」ブロックに **ブロック単位 coalesce** を追加 (§6.4 のマージ規則)。per-field 合成はしない (宛先ブロックは一体の値であり、勝者非 NULL 優先):
...
RLS 追加・grant 変更・index 追加はなし (#98 と同判断)。documents (sales 所有) の変更は一切なし。NULL 意味論: 列 nullable・default 無し。「未設定」= 列 NULL のみで表現 (空オブジェクト保存は Zod refine が拒否 = KMB-E610)。
...
| customers.custom_fields | `zCustomerCustomFields` (順序保持の {label,value} ペア配列。max 50・ラベル重複禁止) | §5.2 (v1.3/#98)。DDL check は `jsonb_typeof='array'` の構造整合のみ。Zod が唯一の正 |
| customers.billing_info | `zCustomerAddressBlock \| null` ({postal_code, address, tel_e164, name, suffix} — `.strict()`・空ブロック拒否) | §5.2 (v1.4/#113)。DDL check は `jsonb_typeof='object'` の構造整合のみ。NULL = 未設定 |
| customers.shipping_info | `zCustomerAddressBlock \| null` (構造は billing_info と同一。suffix は帳票 site_* には未使用) | §5.2 (v1.4/#113)。DDL check は `jsonb_typeof='object'` の構造整合のみ。NULL = 未設定 |

上記いずれも「Zod が唯一の正 + DDL check は構造整合のみ (#98 で確立した拡張パターン)」に従う。これ以外の crm テーブルに JSONB カラムはない (構造化できる属性は列で持つ原則)。
...
6. won_at (レビュー是正 — #102): facade が `shouldRecordWonAt(to_stage, deal.won_at)` (§4.2 不変条件1 と同一の判定関数) で「このタイミングが won 系ステージへの初到達か」を判定し、該当する場合のみ現在時刻を `crm_reopen_deal` の `p_won_at` に渡す。RPC 側は `won_at = coalesce(v_deal.won_at, p_won_at)` で「既存値があれば絶対に上書きしない」の二重防御のみを担う (isWon の唯一の正は DEAL_STAGE_REGISTRY であり SQL 側に複製しない)。lost 案件の中には「lost に落ちる前に一度も won 系ステージへ到達していなかった (won_at が null のまま失注した)」ものがあり、これを ordered/in_production/delivered/invoiced (won 系) へ再開すると、それは won_at にとって正に「初到達」である — 旧設計は「won_at は RPC 側で SET しない」だったが、このケースで isWon なのに won_at=null という不変条件1違反の行が生まれてしまっていた (レビュー指摘で是正)。paid からの再開時 (won_at は既に非 NULL) や、lost 案件のうち一度は won 系ステージを経てから失注していた案件 (won_at が既に非 NULL) の再開では、coalesce により既存の won_at がそのまま保持される (据え置き)。lost_reason は RPC が null クリア (理由は監査 activity に退避 — deals 行から「一度 lost だった履歴」は直接辿れなくなるが、必要なら activities(code='deal.reopened') から集計可能、という許容)
...
/** 郵便番号 (日本 7 桁・ハイフンなし。v1.4 — Issue #113)。UI 側で normalizePostalCode7
 *  (platform/text.ts — 全角/ハイフン/空白を吸収して 7 桁数字化) を通した後の値が唯一の正 */
export const zPostalCode7 = z.string().regex(/^\d{7}$/, "郵便番号は 7 桁の数字で入力してください (KMB-E610)");

/** 請求先/配送先ブロック (v1.4 — Issue #113)。ブロック内キー {postal_code, address, tel_e164, name, suffix}。
 *  suffix (様/御中): billing_info.name が個人名等で deal.company の有無から自動導出する敬称と一致しない
 *  ケースを明示上書きするフィールド。null = 従来どおり deal.company の有無から導出 (後方互換 — 02-sales §6.1)。
 *  shipping_info では未使用 (帳票 site_* に敬称は印字しない) だが、ブロック構造を billing/shipping で共通化する
 *  既定方針を優先し同一スキーマとする (UI 上は配送先セクションに suffix 入力欄を出さない)。
 *  全欄 null の空ブロックは refine で拒否 (KMB-E610) — 「未設定」は列 NULL のみで表現する */
export const zCustomerAddressBlock = z.object({
  postal_code: zPostalCode7.nullable(),
  address: z.string().max(190).nullable(),   // 190 + "〒123-4567 "(10) = 200 = documents 側 Zod 上限 (02-sales §6.1 合成境界)
...
export const zCustomerUpdateInput = z.object({
  ...
  // 請求先/配送先 (v1.4/#113)。custom_fields と同じ「必須・default なし」規約 (stale クライアントの
  // silent wipe 防止。送らない旧 Sheet は KMB-E101 で保存失敗 → 再読み込みで解消)。NULL = 未設定
...
export type CustomerDetail = CustomerListItem & {
  ...
  billing_info: CustomerAddressBlock | null;  // v1.4/#113 追加 (請求先ブロック。NULL = 未設定)
  shipping_info: CustomerAddressBlock | null; // v1.4/#113 追加 (配送先ブロック。NULL = 未設定)
};
...
- 実体: `crm_merge_customers` RPC (§2.2) — 単一 TX で ① deals/tasks の customer_id 付け替え ② activity_links の付け替え (勝者側に同一 activity のリンクがある行は削除 = 重複防止) ③ 勝者の空欄補完 (email/tel/かな/住所/会社/custom_fields (v1.3) + **billing_info/shipping_info (v1.4/#113)** — 勝者の非 NULL 値優先) ④ 敗者を archived + merged_into ポインタ
- **請求先/配送先のマージ規則 (v1.4/#113)**: `billing_info`/`shipping_info` は **ブロック単位 coalesce** (`coalesce(billing_info, v_loser.billing_info)`)。宛先ブロックは一体の値であり per-field 合成 (勝者の name + 敗者の address 混在等) はしない — 勝者が NULL のときのみ敗者ブロックを丸ごと採用する。custom_fields のラベル単位マージとは扱いが異なる (custom_fields はラベルキーで append、宛先は 1 値で coalesce)
...
2. `ACTIVITY_PAYLOAD_SCHEMAS[activity_type].parse(input.payload)` — 失敗 E604 (二段階 parse の内側)
2b. `activity_type === 'email' && payload.direction === 'inbound'` → **E604** (J7 Phase 2 段階解禁 —
    issue #101。outbound は解禁済みのためここを通過する。判定は payload parse **後** — 手順の順序を
    入れ替えた理由: 旧実装は type だけで一律拒否していたため outbound も含め全滅していた)
...
| updateCustomerAction | zCustomerUpdateInput + expectedUpdatedAt (billing_info/shipping_info は AddressBlockFormInput で受け、tel_raw→E.164・postal 正規化のうえ zod parse) | updateCustomer | /admin/customers, /admin/customers/[id] |
| lookupPostalAddressAction (v1.4/#113) | `postalRaw: string` (Zod なし — normalizePostalCode7 で正規化。不能なら KMB-E610) | facade 非経由 (zipcloud プロキシ — §7.1 直下参照) | なし (read-only) |
...
**lookupPostalAddressAction (v1.4/#113 — 郵便番号→住所の自動補完プロキシ)**: `(postalRaw: string) => Promise<Result<{ address: string }>>`。zipcloud (`https://zipcloud.ibsnet.co.jp/api/search?zipcode=...`、無料・登録不要・キー不要) への fetch を **Server Action 1 箇所に隔離** (API 乗換点を 1 箇所に閉じる)。手順: ① `requireAdmin()` → ② `normalizePostalCode7(postalRaw)` が null なら `{ ok:false, code:"KMB-E610" }` → ③ `fetch(..., { signal: AbortSignal.timeout(5000), cache: "no-store" })` → ④ status≠200 / results=null・空 / fetch throw / timeout はいずれも `{ ok:false, code:"KMB-E611" }` (degrade — 手入力で保存は常に成立) → ⑤ `results[0]` の address1+address2+address3 連結を `{ address }` で返す (複数件は先頭採用)。**保存経路 (updateCustomer) とは完全に独立** — 補完の成否は保存の可否に一切影響しない。zipcloud へ送るのは郵便番号のみ (氏名・住所は送信しない — §個人情報)。
...
- **請求先/配送先 (billing_info/shipping_info — v1.4/#113)**: 編集 Sheet の「住所」Field 直後・「状態」の前に **2 セクション**を常設表示 (折りたたまない)。
  - 配置 (§4.2 ワイヤーフレーム): `請求先` = 名前 (maxLength 80) + 敬称セレクト (様/御中/自動 — 自動 = null) + 郵便番号 (例 8600801) + 「住所検索 ⟳」ボタン + 住所 (maxLength 190) + 電話番号 (生入力 090-… 可)。`配送先 (施工先)` = suffix 以外の 3 項目 (敬称欄なし — 帳票 site_* に敬称は印字しないため)
  - **「基本情報と同じ内容をコピー」ボタン** (各セクション右上): `name ← form.name` / `address ← form.address` / `tel_raw ← form.tel_raw` を即時上書き (確認ダイアログなし)。**postal_code と suffix はコピー対象外** (基本情報に該当項目が無いため — help text に明記)。基本情報 address が 190 字超なら 190 字で切詰めてコピーし muted テキストで 1 行注記
  - **郵便番号自動補完** (§4.4): 郵便番号 Input が正規化後 7 桁到達の瞬間 (debounce 500ms) かつ住所欄が空のときのみ自動発火。「住所検索」ボタン押下時は住所欄入力済みでも上書き。補完中はボタンを spinner+disabled。応答適用は「発火時の郵便番号 === 現在値」の場合のみ (レース対策)。失敗 (E610/E611) は muted 1 行「住所を自動入力できませんでした。手入力してください」で、入力・保存は一切ブロックしない。呼び出しは `lookupPostalAddressAction` (§7.1)
  - form state: `AddressBlockFormInput = { postal_code, address, tel_raw, name, suffix }`。`CustomerUpdateFormInput` は billing_info/shipping_info を `AddressBlockFormInput | null` で保持し tel_raw を分離。保存時は `updateCustomerAction` が (a) trim → (b) tel_raw を `normalizeTelOrNull` (失敗はブロック別ラベル付き早期リターン: 「請求先の電話番号の形式が正しくありません。」/「配送先の…」) → (c) postal_code を `normalizePostalCode7` → (d) 全欄 null ならブロック null、の順で正規化してから zod parse
  - **クライアント側二重防御**: 純関数 `collectAddressBlock` を export しテスト対象化 (郵便番号が非空かつ正規化後 7 桁でない → 保存中断。名前だけ等の部分入力は許容)。サーバー側の保険は `customerUpdateErrorDetail` の billing_info/shipping_info 分岐 (§3 エラーコード — 生 Zod JSON 露出防止)
  - 新規作成フォーム (`CustomerForm.tsx`) には載せない (#98 と同じ裁定。作成後に編集 Sheet で追記)

**顧客詳細 (`customers/[id]/page.tsx`) — 2 カラム**

- 左 (プロフィール): 基本情報カード (編集 Sheet を開く「編集」ボタン) / 進行中案件リスト (DealListItem、行クリックで案件詳細へ) / open タスクリスト (チェックで complete) / 操作 dropdown-menu (「重複を統合」→ 統合 Dialog (相手を command 検索 → 確認 →mergeCustomersAction)、「アーカイブ」)。基本情報カードの dl グリッドは「住所」の後に custom_fields を追加順で列挙 (v1.3/#98。0 件なら何も出さない)。custom_fields の後に **「請求先」「配送先」** を追加 (v1.4/#113 — `CustomerProfileCard`。null ブロックは行ごと非表示。郵便番号は formatPostalCode7 で "860-0801" 表示)
...
| KMB-E604 | activity payload が契約と不一致 ('email' の inbound 挿入含む — outbound は #101 で解禁済み) | 記録の形式が不正です | 発生源モジュールの不具合 — ログの detail を確認 (admin 操作では通常発生しない) |
...
| KMB-E610 (v1.4 — #113) | 郵便番号形式不正/空の宛先ブロック (§5.2 zPostalCode7・zCustomerAddressBlock refine) | 郵便番号は7桁の数字で入力してください | ハイフンなし7桁で入力。宛先を消す場合は全欄を空にして保存 |
| KMB-E611 (v1.4 — #113) | 郵便番号からの住所自動補完に失敗 (zipcloud 障害/該当なし/timeout — §7.1 lookupPostalAddressAction) | 住所を自動入力できませんでした | 住所を手入力して続行 (保存は検索と無関係に可能) |
| E612〜E619 | — (予約。帯内追加は契約書更新が先) | — | — |
...
| tests/crm-activity-payloads.test.ts | 9 type × 正常 parse (email は #101 拡張フィールド込み) / 各 type の必須欠落・未知キー (.strict()) 拒否 / zAppendActivityInput の links 0 件・2 対象リンク拒否 / ref_table・ref_id 片側 NULL 拒否 |
| tests/crm-append-activity-email.test.ts (#101) | crmFacade.appendActivity('email') — direction='inbound' は KMB-E604 拒否 (client 解決前に短絡することを確認) / direction='outbound' は受け入れて INSERT + リンクまで進む |
...
- 'email' activity の outbound (帳票のメール送付) は issue #101 で実装済み (appendActivity の拒否分岐を direction 判定に変更 + activity-timeline.tsx にレンダラ追加)。残る拡張余地は inbound (受信取込 — BCCログ等): DB check・Zod enum の型は既に inbound/outbound 両対応済みのため、受信基盤を用意すれば appendActivity の拒否分岐を外すだけで済む
...
| 版 | 日付 | 内容 |
|---|---|---|
| v1.4.1 | 2026-09-06 | 実装是正 (契約・DDL 無変更)。**§5.2/§6.2 listCustomers**: q が `normalizeJpPhoneToE164` で正規化できる場合は `tel_e164 like '<E.164>%'` の前方一致を名前/かな/email の部分一致と OR で併用 (repository `CustomerListQuery.telE164Prefix` 内部拡張)。listCompanies/listCustomers の `.or()` フィルタ値は `src/lib/postgrest-filter.ts` の `ilikeContainsFilter` で引用符囲み (カンマ・括弧を含む検索語で PGRST100 にならない)。**§6.1 completeTask/cancelTask**: task_event(completed/cancelled) を createTask と同様に task の customer_id/deal_id へ activity_links で紐づける。**§6.7 relinkActivity**: session 実行時は `platformFacade.isAdmin` を確認し非 admin は KMB-E202 (契約表の E202 を実装)。**§6.5/§12.1 取込**: archived 顧客の復帰 lifecycle も新規作成時と同じ規則 (`createDeal:false` は 'customer'、既定は 'lead') |
| v1.4 | 2026-07-16 | **顧客の請求先/配送先 (Issue #113)**。**§2.2**: migration `20260715000002_customers_billing_shipping.sql` を追記 — `customers.billing_info/shipping_info jsonb` (nullable・default 無し) + 各 `jsonb_typeof='object'` check、`crm_merge_customers` の空欄補完ブロックに `billing_info/shipping_info` の**ブロック単位 coalesce** を追加 (per-field 合成禁止)。**§2.6**: JSONB 対応表に custom_fields/billing_info/shipping_info の 3 行を正記 (「JSONB カラムはない」の stale 記述を是正)。**§5.2**: `zPostalCode7` (7 桁数字) と `zCustomerAddressBlock` ({postal_code, address(max190), tel_e164, name(max80), suffix(様/御中)}・`.strict()`・空ブロック refine 拒否 = KMB-E610) を新設、`zCustomerUpdateInput` に `billing_info/shipping_info` を必須 (default なし) で追加。**§5.3**: `CustomerDetail` に `billing_info/shipping_info: CustomerAddressBlock \| null` を追加。**§6.4**: マージのブロック単位 coalesce 規則を明記。**§7.1**: `lookupPostalAddressAction` (zipcloud プロキシ・requireAdmin・5s timeout・E610/E611 degrade・保存経路と独立) を追加。**§8.2**: 編集 Sheet に「請求先」「配送先」2 セクション (コピー ボタン・郵便番号自動補完 debounce+レース対策・suffix 選択・collectAddressBlock クライアント側二重防御) を新設、基本情報カード (`CustomerProfileCard`) に表示。**§9**: KMB-E610 (郵便番号形式不正/空ブロック) / KMB-E611 (住所自動補完の失敗) を追加、予約帯を E612〜E619 に更新。跨モジュール射影 (CustomerRef/DealRef の billing/shipping) は 07-contracts-delta v1.10、帳票複製規則 (composePostalAddress/deriveBillingFields/previewBillingFields) は 02-sales §5.2/§6.1/§8.3。00-overview §3.3 に KMB-E610/E611 登録済み。canonical 契約 (zCustomerInput / CrmFacade シグネチャ) 無変更 |
