...
| 25 (Issue #102 — 実装フェーズの契約差分。J1〜J12 の並列執筆裁定とは時期が異なるが J10「契約差分は本書に集約」に従い記録) | 01-crm §4.2-8 (終端ステージの案件再開 reopenDeal) | `zReopenDealInput` (crm/contracts.ts) と `CrmFacadeExtended.reopenDeal` を追加してよいか / D7 §4.10 canonical 昇格の要否 | **採用 (契約外拡張のまま — D7 昇格しない)** | `zMarkDealLostInput`/`markDealLost` (裁定 J1〜J12 期の既存前例) と同型の判断基準: 呼び出し元は crm 自身の admin UI (`/admin/deals` Server Actions) のみで他モジュールから参照されない → 01-crm.md §5.2「契約外拡張スキーマ」・§6.2「契約外拡張メソッド」に定義 (完全定義は 01-crm §5.2 参照)。KMB-E609 は帯内追加 (00-overview §3.3 に登録) |
