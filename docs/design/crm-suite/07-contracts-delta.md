...
| 21 (05 v1.1) | 05-site-settings §2.5-4 (レビュー反映) | 02-sales への申し送り: migration 0028 の SQL ヘッダに「0035 適用済み環境では適用禁止 (0035 が 0028 を包含)」コメントを追加し、02-sales §2.3.3 に 05 §2.5 への相互参照を置く | **採用 (申し送り — 02-sales 反映は Issue 起票時)** | 0035 は 0028 を包含する media 参照 3 点セットの全文置換。settings フェーズは sales と独立着手可のため 0035 先行があり得、逆時系列で 0028 を適用すると favicon 参照ガードが DROP+CREATE で消失し ai-draft favicon media の日次 cleanup 物理削除事故に至る (05 §2.5-4・§9 パターン 10)。事故検知は 05 受入 A1 (本番 pg_policies 実測) |
