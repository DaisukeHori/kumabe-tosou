...
- **types**: `DataTableShell` 一覧 (色スウォッチ / label / key / 拘束・非拘束 Badge / 既定 h / 並び順 / 有効)。行 Enter or クリック → 編集 Dialog (react-hook-form + zWorkTypeInput)。色入力は `popover` 内プリセットパレット 12 色 + hex Input。新規作成後の key は変更不可 (表示のみ — ブロック/テンプレの参照キーのため)。削除は E702 時に「使用中のため無効化してください」トースト
