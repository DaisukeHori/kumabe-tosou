# 山岸塗装 CRM スイート — 通販シミュレーター設計書 (06-simulator)

- 版: v1.3 (2026-07-11: #38 実装・本番検証で判明した真因を反映し §2.2 に訂正ノートを追記 — 障害メカニズムは「Data Cache 焼き付き」ではなく「unstable_cache 内での cookie 依存 client 使用によるランタイム例外」だった。末尾の更新履歴参照)。旧: v1.2 (2026-07-11: §17/§16 の「00 §13 Phase 4 合算 2,200 と整合」宣言を撤回し、00-overview v1.2 の改訂値 〜3,500 (05+06 合算) に追随)。旧: v1.1 (2026-07-11: レビュー指摘反映 — 末尾の更新履歴参照)。初版 v1.0 同日
...
模型好きの会社員が、夜、スマホで山岸塗装の SHOP ページを開く。「サイズ × 個数 × グレード。3つ選べば、概算が出る。」と見出しは出ているのに、その下には**「価格はお問い合わせください。」の一行だけ**。選ぶボタンも金額も出ない。実は古いホームページ (引っ越し前のもの) が今も残っていて、そちらでは計算機がちゃんと動く。新しいページの奥には正しい価格表がきちんと入っているのに、**引っ越し初日に「まだ値札がありません」と印刷された案内板が、そのまま貼りっぱなし**になっている — それが今の状態である。
...
> ⚠️ **訂正 (2026-07-11, #38 実装時に判明)**
>
> 本節の「Data Cache 焼き付き」説は **誤りだった**。#38 の実装・本番検証で判明した真の障害メカニズムは次のとおり:
>
> `pricing/repository.ts` の `getPriceTable` が `createSupabaseServerClient` (Next.js `cookies()` に依存するクライアント) を使っており、これが `facade.ts` 側で `unstable_cache()` にラップされていた。Next.js は `unstable_cache` 内で `cookies()`/`headers()` を呼ぶと実行時例外を投げる仕様のため、公開経路 (`getActivePriceTable()` = `activeOnly=true`) は毎回この例外を起こし、`facade` がそれを `{ok:false}` に握りつぶして fallback 表示 (「価格はお問い合わせください。」) に恒常的に落ちていた。データ焼き付きではなく、**呼び出すたびに例外→fallback を再生成し続けていた**というのが実態である。
>
> したがって、以下 §2.3 の切り分け手順・§2.4 の revalidate/TTL 前提の恒久策は、上記の真因には効かない (無害だが無関係)。実際に修理を成立させた変更は、公開読み取り経路 (`activeOnly=true`) を cookie 非依存の `createSupabasePublicClient` に切替えたことである (PR #67、commit `53c2d9c`。admin 経路 `activeOnly=false` は従来どおり `createSupabaseServerClient` のまま)。
>
> ただし §2.4 P1 (`export const revalidate = 3600`) / P2 (`unstable_cache` の `tag='prices'` + TTL) 自体は、真因修正後も「価格改定の即時反映」と「万一の自己修復」の安全網として引き続き有効であり、残置してよい。
>
> 以下は当時の (誤った) 分析を履歴として残す。

```
時系列 (推定・F1〜F6 と整合する唯一の説明。2026-07-11 #38 実装時にこの分析は誤りと判明 — 上記訂正参照):
...
| 版 | 日付 | 内容 |
|---|---|---|
| v1.3 | 2026-07-11 | **§2.2 訂正**: #38 の実装・本番検証 (PR #67) で判明した真因を反映。障害メカニズムは「Vercel Data Cache の焼き付き」ではなく、「`pricing/repository.ts` の `getPriceTable` が `createSupabaseServerClient` (cookies() 依存) を使い、それが `facade.ts` で `unstable_cache()` にラップされていたため、公開経路で毎回ランタイム例外→facade が `{ok:false}` に握りつぶして fallback 表示になっていた」という構造バグだった。§2.3/§2.4 の revalidate・TTL 前提の手順は真因には効かず、実際の修理は公開読み取り経路を `createSupabasePublicClient` に切替えたこと (PR #67)。P1 (`revalidate=3600`) / P2 (`tag='prices'`) 自体は将来の即時反映・自己修復の安全網として有効なため残置。旧記述は §2.2 内に履歴として保持 |
