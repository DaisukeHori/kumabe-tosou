# 山岸塗装 CRM スイート 全体アーキテクチャ設計書 (00-overview)
...
| 項目 | 理由・扱い |
|---|---|
| メール連携 (BCC ログ / 受信取り込み) | 裁定 J7。帳票のメール送付 (PDF 添付・outbound) は issue #101 で実装済み — 対象外なのは BCC ログと受信取込 (inbound) のみ。activity_type `'email'` の inbound 方向は引き続き v1 挿入禁止 (KMB-E604) |
...
| J14 | 左ナビの大項目グルーピング化 (6 セクション折りたたみ + 現在地追従) | §2.4 / #94 |
...
### 2.4 ナビゲーション追加 (`src/app/admin/nav-items.ts`、裁定 J11 / グループ化は J14)

`ADMIN_NAV_ITEMS` は J11 時点ではフラット配列だったが、#94 (裁定 J14) で 6 セクションの
`ADMIN_NAV_GROUPS` へ再編し、`ADMIN_NAV_ITEMS` はその `flatMap` による後方互換 derived
export になった (`admin-nav.tsx` が折りたたみ UI で描画・現行グループ構成は以下)。

- (グループ外・最上部固定) ダッシュボード `/admin`
- 顧客管理 (`crm`): 顧客 `/admin/customers`・案件 `/admin/deals`・やること `/admin/tasks`
- 営業・予定 (`sales-ops`): 帳票 `/admin/documents`・カレンダー `/admin/calendar`・通話 `/admin/calls`
- ホームページ (`content`): 施工事例 `/admin/works`・記事 `/admin/posts`・お客様の声 `/admin/voices`・メディア `/admin/media`・ビジュアル編集 `/admin/visual`
- サイト運営 (`site`): 価格表 `/admin/prices`・問い合わせ `/admin/inquiries`・チャネル管理 `/admin/channels`・サイト設定 `/admin/settings`
- システム (`system`): AIスタジオ `/admin/studio`・利用料金 `/admin/costs`

**今後 nav-items を追加する場合は該当グループの `items` 配下へ追加すること** (フラット配列
への直接追加は不可。#41〜#63 の後続 Issue はこのグループ構造を前提にする)。業務系統が最も
近いグループに 1 行追加するのが原則 (例: crm 系画面 → `crm` グループ)。既存 6 グループに
収まらない新カテゴリが必要な場合のみグループ自体を追加し、その際は
`tests/admin-nav-groups.test.ts` の href 集合も更新する。settings 拡張は既存「サイト設定」
タブ内 (新ナビなし)。simulator は公開サイト側 (ナビ変更なし)。middleware matcher は
`/admin/:path*` のまま変更不要 (J11)。shadcn CLI で `dropdown-menu` / `popover` /
`calendar` / `date-picker` / `command` を追加する (各モジュール Issue に明記)。折りたたみ
状態はブラウザ localStorage (`yamagishi-admin-nav-collapsed:v1`) のみで永続化する (サーバー
保存なし、#94)。
...
| `email` | sales facade (帳票のメール送付 — issue #101、J7 段階解禁) | `zEmailActivityPayload` (direction / subject / to / document_id / doc_no / version / provider_message_id) | ✅ outbound のみ。inbound (受信取込) は引き続き v1 挿入禁止 (KMB-E604) |
...
| KMB-E609 | crm | 案件の再開が不正 (終端以外からの再開/再開先が非終端7ステージ外/理由なし — 01-crm §4.2 v1.2 reopenDeal、Issue #102) |
| KMB-E610 | crm | 郵便番号形式不正/空の宛先ブロック (請求先/配送先 — 01-crm §5.2 zCustomerAddressBlock、Issue #113) |
| KMB-E611 | crm | 郵便番号からの住所自動補完に失敗 (zipcloud 障害/該当なし — degrade して手入力継続。01-crm §7.1 lookupPostalAddressAction、Issue #113) |
...
| KMB-E644 | sales | 帳票メール送付の送信失敗 (Resend APIエラー / RESEND_API_KEY 未設定 — issue #101) |
| KMB-E645 | sales | 帳票メール送付の宛先メールアドレス不正・未指定 (issue #101) |
...
終端:        paid / lost からは updateDealStage / markDealLost では一切遷移不可 (KMB-E602)。
             復帰は reopenDeal 専用経路のみ (KMB-E609、admin専権・理由必須・system activity監査。
             v1.2 — Issue #102。01-crm §4.2 が canonical)
...
| sales | 02-sales.md | **0026〜0028, 0036** (0036 = document_emails — issue #101) | KMB-E620〜E649 | documents, document_lines, payments, document_sequences, issued_documents, print_tokens, pdf_render_lock, document_revision_stagings (v1.2 — 02-sales v1.1 §2.3.2 の service 専用補助 3 テーブル), document_emails (issue #101 — 帳票メール送付の送信台帳) (+bucket issued-documents, branding-assets) | 帳票 |
...
1. 山岸塗装の事業形態 (法人/個人) — **Twilio 日本番号は法人限定**。個人なら Twilio へ直接確認 (設計は番号非依存で吸収済み)
...
| 版 | 日付 | 内容 |
|---|---|---|
| v1.4 | 2026-07-16 | #113 (顧客の請求先/配送先): §3.3 に **KMB-E610** (郵便番号形式不正/空の宛先ブロック) と **KMB-E611** (郵便番号→住所自動補完の失敗 — degrade) を crm 帯 (E601〜E619) 内で追加登録。errors.ts への実登録・01-crm §9 表への詳細化と同期 |
| v1.3 | 2026-07-15 | #94 (裁定 J14): §1.1 に J14 行追加・§2.4 を `ADMIN_NAV_ITEMS` フラット配列前提の記述から `ADMIN_NAV_GROUPS` (6 セクション・折りたたみ) 前提に更新し、今後の nav-items 追加手順を「該当グループの items 配下へ」に是正 (誤ってフラット追加されるのを防止) |
