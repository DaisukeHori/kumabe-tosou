...
│  └── Route Handlers (/api/**)                                          │
│        ├── /api/ai/*      … Claude API 呼び出し (advance 駆動 + SSE 観測)│
│        ├── /api/transcribe … OpenAI STT 呼び出し                       │
│        ├── /api/publish/* … 配信予約 API                               │
│        └── /api/jobs/*    … pg_cron 起床の worker (202 即応 + after)   │
└───────────────┬───────────────────────────────────┬───────────────────┘
                │                                   │
        ┌───────▼────────────  Supabase  ───────────▼───────┐
        ...
        │ pg_cron … 毎分の起床信号 → Vercel /api/jobs/*        │
...
本節は **M0 以前の既存 CMS テーブル群 (下記 §2.2 に DDL 全文を持つ 22 テーブル) の単一ソース** であり、これらについては migration ファイルを本節から生成し、乖離した場合は本節を正とする。

**crm-suite (crm / sales / scheduling / telephony) の新規テーブルについては本節は DDL の正ではない。** それらの DDL canonical は各モジュール設計書 (`docs/design/crm-suite/01-crm.md` / `02-sales.md` / `03-scheduling.md` / `04-telephony.md`) 側にあり、本節は §2.1 ER 概観と §2.2 末尾の参照台帳でテーブル名を列挙し所有と DDL 正の所在を指すのみとする (00-overview.md §3.4 の「document_sequences は sales 所有」参照併記スタイルを踏襲。二重管理を避けるため)。
...
--- crm-suite (Phase 0〜5 で増築。DDL canonical は各モジュール設計書。00-overview.md §2.2 依存方向図と整合) ---

crm      : companies ─┐
                      ├── customers ── deals ── activities ── activity_links
                      │                  └── tasks
sales    : documents ── document_lines / payments / issued_documents (append-only)
           document_sequences (採番; sales 所有・M0 0022 で作成)
           print_tokens / pdf_render_lock / document_revision_stagings (service 専用補助)
           document_emails (帳票メール送付台帳; append-only)
scheduling: work_types ── work_templates ── work_template_items
           work_blocks / calendar_connections ── calendar_event_links
telephony: calls ── call_recordings / call_jobs

依存方向 (00-overview §2.2): sales→crm / scheduling→crm / telephony→crm+ai-providers。
sales ⇄ scheduling は相互参照禁止 (受注→ブロック生成は app 層合成)。
```
...
-- 共通規約:
-- 1) updated_at を持つ全テーブルに set_updated_at trigger (moddatetime) を付与する。
-- 2) DB check は enum/status/非負など構造的制約に限定。文字数上限・regex 等の値制約は
--    Zod (契約書 §4) が唯一の正 — parity テストの対象も enum/status の一致に限る (§11)。
-- 3) channel_posts.channel は参照先 channel_drafts.channel との一致を trigger で検証。
-- 4) channel_drafts.content の default '{}' は status='generating' 中のみ許容される
--    プレースホルダで、needs_review への遷移時に CHANNEL_CONTENT_SCHEMAS で検証してから進める。
```

#### 2.2.1 crm-suite 新規テーブル参照台帳 (DDL canonical は各モジュール設計書)

crm スイート (Phase 0〜5) で追加された新規テーブルは以下 **24 本**。所有・DDL の正は 00-overview.md §10 の割当表 (各モジュールへの発注仕様) と各モジュール設計書であり、本節は台帳として名前・所有・migration・DDL 正の所在を指すのみ (DDL 全文は複製しない)。

| モジュール | テーブル | migration | DDL canonical |
|---|---|---|---|
| M0 共通基盤 | document_sequences (採番。所有は sales) | 0022 | 00-overview.md §3.4 |
| crm | companies, customers, deals, activities, activity_links, tasks (6) | 0023 | 01-crm.md §2 |
| sales | documents, document_lines, payments (3) | 0026 | 02-sales.md §2 |
| sales | issued_documents, print_tokens, pdf_render_lock, document_revision_stagings (4) | 0027 | 02-sales.md §2 |
| sales | document_emails (帳票メール送付台帳・append-only。issue #101) | 0036 | 02-sales.md §18 |
| scheduling | work_types, work_templates, work_template_items, work_blocks (4) | 0029 | 03-scheduling.md §2 |
| scheduling | calendar_connections, calendar_event_links (2) | 0030 | 03-scheduling.md §2 |
| telephony | calls, call_recordings, call_jobs (3) | 0032 | 04-telephony.md §2 |

- **本数の根拠**: 00-overview.md §10 の「所有テーブル」欄が実テーブルリストの正 (物理テーブル総数 = document_sequences 1 + crm 6 + sales 8 + scheduling 6 + telephony 3 = **24**)。
- **§13 の「新規テーブル 20」との食い違いに注意**: 00-overview.md §13 の規模見積り総表の「20」は 07-contracts-delta 裁定記録 v1.8 (print_tokens / pdf_render_lock / document_revision_stagings を sales 所有表に追記) 反映前の stale 値であり、さらにその後 document_emails (issue #101) が追加されて実数は 24 になった。§10 の割当表が正。
- **Storage バケット**: issued-documents / branding-assets (sales) / call-audio (telephony) を新設 (詳細は各モジュール設計書)。
- 顧客テーブルの後日拡張列 (customers.custom_fields / billing_info / shipping_info — migration 0036/20260715 系, issue #98) と ai_runs.style_profiles 列追加 (0036) は既存テーブルへの ALTER であり新規テーブルには数えない。
...
| channel_drafts.claims | zClaim[] (§4.3。生成出力の契約は zChannelDraftOutput §4.4) |
...
|---|---|---|---|---|---|
| `media-originals` (非公開) | ✗ | 署名付き URL | 署名付きアップロード URL (TTL 5 分) | 参照ゼロ検証 (E301) 通過時のみ | 無期限 |
| `media` (公開 — レンディション専用) | ✓ (WebP / JPEG レンディションのみ。**原本から sharp で再エンコードして生成するため EXIF/GPS は自動除去**) | ✓ | サーバ処理のみ (原本から自動生成、直アップロード不可) | 原本削除に追従 | 無期限 |
| `audio` (非公開) | ✗ | 署名付き URL (TTL 60 分) のみ | 署名付きアップロード URL | 自動 | **90 日で自動削除** (pg_cron 日次)。raw_text / cleaned_text は DB に永続のため失われない |
...
| /api/ai/runs/[id]/advance | POST | admin | — | { status } (1 呼び出し = 1 stage) | 409 (lease 中), E401-404 |
| /api/ai/runs/[id]/stream | GET | admin | — | SSE (zRunProgressEvent) **観測専用** | E402 |
| /api/ai/drafts/[id] | PATCH | admin | zEditDraftReq | { revision } | E101, E103 |
| /api/ai/drafts/[id]/approve ・ /reject | POST | admin | — | { ok } | E101 |
| /api/ai/drafts/[id]/regenerate | POST | admin | zRegenerateReq | { revision } | E401-404 |
| /api/publish/schedule | POST | admin | zScheduleReq (draft 単位) | { post_ids } | E101, E505 |
| /api/oauth/x/start ・ /callback | GET | admin (+state cookie) | — | 302 | E201, E501, E503 |
| /api/oauth/meta/start ・ /callback | GET | admin (+state cookie) | — | 302 | E201, E502, E503 |
| /api/revalidate | POST | **shared secret ヘッダ** (`x-revalidate-secret`) | zRevalidateReq | { revalidated } | 401 |
| /api/jobs/publish | POST | **shared secret** (pg_cron → `net.http_post`) | — | **202 即応** (本体は after() で実行) | E5xx (DB 記録) |
| /api/jobs/watchdog | POST | shared secret (pg_cron 5 分毎) | — | 202 | — |
...
- **読み出し**: worker route (/api/jobs/publish、service role) のみ (`vault.decrypted_secrets` を RPC 経由で参照)。クライアントコンポーネント・公開ページ・admin UI からの到達経路を作らない (worker は shared secret ガード済み)。
...
scheduled → publishing → published
    │            │
    │            ├──→ failed (API がエラーを確定応答) ──(手動リトライ)──→ scheduled
    │            ├──→ manual_required (note 常時 / X・IG の結果不明失敗 E506)
    │            │        └─ 人間照合: 投稿済み → published / 未投稿 → scheduled へ戻す
    └──→ cancelled
```
...
| `publishing` | worker が API 呼び出し中。**10 分超停滞は watchdog が manual_required (E506) に倒す** — 結果不明のため自動 failed 化も自動再開もしない (§8.2 の at-least-once + 人間照合モデル) |
...
- 生成 (stage 2-4) は **「1 HTTP 呼び出し = 1 stage」** に分割 (POST /api/ai/runs/[id]/advance)。各 stage 単体が Vercel maxDuration 300s に収まる粒度 (extract ≈30-60s / research ≈60-90s / draft 1 呼び出しで 4 チャネル並列 ≈60-120s)。SSE (/stream) は**観測専用**で実行を担わないため、run 全体の所要が 300s を超えても破綻しない (Codex 指摘対応)。
- 配信 (stage 6) は **Next.js Route Handler /api/jobs/publish** (Deno Edge Function は facade を呼べないため不採用)。pg_cron `* * * * *` → `net.http_post` (shared secret) で起床 → worker は**即 202 応答**し `after()` で本体処理 (pg_net の数秒 timeout に依存しない)。1 回の起動で最大 5 件処理 (X rate limit 保護)。

### 7.6 stage 実行の lease 意味論 (切断・再開)

旧設計 (status CAS のみ) はクラッシュ時に status が実行中のまま残り、再接続が「監視モード」に落ちて永久停滞する欠陥があった (Codex 指摘)。lease 方式に置き換える:
...
- draft の `thread[]` は生成時点で X の重み付き字数 (上限 280) を検証。字数計算は自作せず**公式 `twitter-text` ライブラリの parseTweet を薄く包む** (X 公式推奨 — Codex 指摘で自作案から変更)。zXTweet (契約書 §4.4) の refinement として適用、超過は KMB-E404 で自動再生成 1 回。
...
| コード | カテゴリ | 意味 | 復旧アクション |
|---|---|---|---|
| KMB-E001 | 0xx 横断集計 | 管理ナビの未対応件数の集計失敗 (DB 障害・部分失敗。nav-badges — #129 R6c で新設) | バッジ非表示に縮退 (ナビは正常)。ログの detail 確認。再読み込みで回復 |
| KMB-E002 | | 管理ナビの未対応件数の集計タイムアウト (性能予算超過) | バッジ非表示に縮退 (ナビは正常)。頻発時は DB 応答とバッジ集計クエリを確認 |
...
| フェーズ | 必須テスト (完了条件に含む) |
|---|---|
| 1a | migration 適用 (clean → up)、RLS マトリクス全セル (§3.2)、Storage バケット認可 (§3.4)、`contracts-ddl-parity.test.ts` (**enum/status の一致に限定** — 長さ/regex は Zod が正、§2.2)、admin bootstrap スクリプト + signup 無効化確認、rate_limits 動作、seed 照合 (A7) + ロールバック検証 (§12.1) |
...
| 2a | lease 取得競合 (advance 二重呼び出し → 409)、lease 失効後の同 stage 再実行 (成果物 UPSERT 冪等)、stage_attempts 上限、SSE snapshot 復元 (§7.6)、watchdog、25MB 分割 |
| 2b | diff 単体 (§10; 巨大 diff・絵文字)、revision 履歴、承認フロー E2E、inference マーカー表示 |
| 2c | OAuth モック (state 不一致・token 交換失敗、X/Meta 両方)、refresh ローテーション競合 (advisory lock)、結果不明失敗 → manual_required (E506) と人間照合フロー、スレッド途中失敗再開 (E504)、課金ガード (E505、予定分込み合算)、twitter-text ラッパの境界値 |
...
3. **seed スクリプト** (`scripts/seed-from-legacy.ts`): Phase 0 のハードコードコンテンツ (works 6 / voices 3 / notes 7 記事 / PRICE_TABLE / 会社情報 / 画像 14 枚) をパースして DB + Storage へ投入。全件 `status='published'`、`is_placeholder=true`。
   - **スナップショット + ロールバック** (一括書き込み規約適用): DB 投入は 1 トランザクション + `seed_manifest` に記録。**Storage は Postgres トランザクションに参加できず原子性はない** (Codex 指摘で明示) — manifest による補償削除 (compensating cleanup) を正式手順とし、`scripts/rollback-seed.ts` が manifest 逆順に DB → Storage の順で削除する (Storage 削除失敗は再実行可能)。
...
- B1: 5 分音声 → 4 チャネル draft 生成が **10 分以内に ready_for_review 到達 (目標値であり SLA ではない)**。stage 分割実行 (§7.5) のため単一 Function の maxDuration 300s 制限には構造的に抵触しない。
...
| admin 初期表示 | ≤ 3s | 手動計測 |
| AI 実行 (5 分音声 → 4 チャネル) | ≤ 10 分 (B1、目標値) | ai_runs のタイムスタンプ |
| 保存 → 公開反映 | ≤ 5s (A4) | E2E |
...
- 週次バックアップは **GitHub Actions** (schedule workflow) で実行: `pg_dump` (接続文字列は GH Secrets) → Storage `backups/` へアップロード (12 週ローテーション)。※ pg_cron は DB 内 SQL 専用で OS コマンドの pg_dump は実行できないため、旧設計 (pg_cron 実行) を訂正 (Codex 指摘)。
...
| v3.5 | 2026-07-17 | crm-suite (Phase 0〜5) 実装完了との文書同期 (#63): §2 冒頭の「本節が DDL の単一ソース」宣言を「M0 以前の既存 22 テーブルの単一ソース + crm-suite 新規テーブルの参照台帳」に是正、§2.1 ER 概観に crm/sales/scheduling/telephony ブロック追加、§2.2.1 に crm-suite 新規 24 テーブル (document_sequences 1 / crm 6 / sales 8 [document_emails 含む] / scheduling 6 / telephony 3) の参照台帳を追記 (DDL canonical は各モジュール設計書)。00-overview §13「新規テーブル 20」は §10 反映前の stale 値である旨を注記 (実数 24 は §10 が正) |
