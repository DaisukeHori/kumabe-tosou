...
export const KMB_ERRORS = {
  // ---------- 0xx 横断集計 (nav-badges — 管理ナビ/ダッシュボードの読み取り専用横断集計。
  // R6c / #129。移行設計.md §6「唯一の facade 追加例外」で新規に割り当てた帯。既存 1xx〜9xx
  // とは番号が衝突しない。失敗時はいずれも「バッジ非表示に縮退」でナビ描画は継続する) ----------
  "KMB-E001": {
    category: "0xx 横断集計",
    message: "ナビの未対応件数を集計できませんでした。件数バッジは表示されません。",
    recovery: "一時的な集計失敗 (ナビ機能自体は正常)。ログ (Vercel) の detail を確認。再読み込みで回復",
  },
  "KMB-E002": {
    category: "0xx 横断集計",
    message: "ナビの未対応件数の集計がタイムアウトしました。件数バッジは表示されません。",
    recovery: "一時的な負荷/遅延 (ナビ機能自体は正常)。頻発時は DB 応答とバッジ集計クエリを確認",
  },
