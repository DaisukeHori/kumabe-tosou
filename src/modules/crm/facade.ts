...
export const crmFacade: CrmFacadeExtended = {
      ...
      if (!opts?.force) {
        ...
        if (dedup.value.kind !== "none") {
          ...
          return {
            ok: false,
            code: "KMB-E601",
            detail: candidates.map((c) => `${c.customer_id}:${c.name}`).join(", "),
          };
        }
      }
  ...
  async listCustomers(rawFilter, rawPagination) {
    try {
      ...
      const companyIds = [...new Set(page.value.items.map((c) => c.company_id).filter((v): v is string => v !== null))];
      const companies = await getCompaniesByIds(supabase, companyIds);
      if (!companies.ok) return companies;
      const companyNameMap = new Map(companies.value.map((c) => [c.id, c.name]));

      const customerIds = page.value.items.map((c) => c.id);
      const openDealCounts = await countOpenDealsByCustomerIds(supabase, customerIds);
      if (!openDealCounts.ok) return openDealCounts;

      const items: CustomerListItem[] = page.value.items.map((c) => ({
        id: c.id,
        kind: c.kind,
        name: c.name,
      ...
      for (const row of page.value.items) {
        ...
        if (!payloadParsed.success) {
          return {
            ok: false,
            code: "KMB-E604",
            detail: `activity ${row.id} の payload が契約と不一致です: ${payloadParsed.error}`,
          };
        }
        items.push({
          id: row.id,
